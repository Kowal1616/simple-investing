...
Simple Investing is a Flask App that aims at promotion of low-cost, low-maintains, long-term investing (Boggleheads style!). My app's main feature is a table which displays long term returns of chosen etfs based portfolios over 5, 10, 20 and 30 years periods. A comparison table is fed with extended historical data (enhanced with corresponding indexes and simulations) and up to date thanks to monthly performed updates.

#### **DATA SOURCES & DISCLAIMER**
Aplikacja wykorzystuje zagregowane dane finansowe pochodzące z wielu niezależnych dostawców (Market Data Providers) oraz autorskie symulacje oparte na historycznych indeksach rynkowych w celu zapewnienia 30-letniej perspektywy inwestycyjnej.

**Disclaimer:** Prezentowane dane mają charakter wyłącznie informacyjny i edukacyjny. Wyniki historyczne i symulacje nie gwarantują przyszłych zysków. Autor nie ponosi odpowiedzialności za decyzje inwestycyjne podjęte na podstawie aplikacji.
...
To minimize the lack of data I decided to use aggregated consensus prices. It collects data from multiple professional market data providers and computes the average from the successful responses, guaranteeing robust and reliable fetching.
...
4. **Set environment variables:**
    You can set these variables in your environment or create a `.env` file in the root of your project and add the variables there. Copy `.env.example` to `.env` and fill the variables:
    - `DATA_PROVIDER_A_KEY`
    - `DATA_PROVIDER_B_KEY`
...
3. **Set environment variables:**
    ...
    In the Codespace terminal, set the environment variables:

    ```
    export DATA_PROVIDER_A_KEY=your_key
    export DATA_PROVIDER_B_KEY=your_key
    ```
